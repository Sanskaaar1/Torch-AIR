...
The user provides:
- A backend name (e.g., `ascend npu`, `habana gaudi`, `openreg`, `spyre`)
...
1. **Read the checklist template** from the appropriate path:
   - Functional: `frameworks/pytorch/checklist.md` (or `checklist_private.md`)
...
After the backend has been located and the evaluation mode determined but **before**
...
checklist templates. Security is an evaluation **dimension** nested inside the
...
- All output files go in `torch-air-report/`.
